Conceitos Basicos - semana 1
- Identidade Eletronica (ler capitulo 2 tese) _ OK
- Nivelamento Linux
    - Assistir aulas Ubantu _ OK
    - Instalar Ubantu _ OK
    - Aula sobre comandos do terminal _ OK (8/8)
    - Exercicios comandos terminal _ (1/2)
      
          1. https://drive.google.com/file/d/1MvgIBejivYdUGKymREoDp08mp6Tc9R38/view?usp=sharing
      
Dimensao de Programacao - semana 2 e 3
- GIT
    - Assistir aulas _ (1/8)
- Docker + Python
    - Ler e reproduzir no Git Lab
- Programacao Python
    - Aula Python 3
    - Exercicios Python 3
    - Aula Flasck e reproduzir exemplos
- Python + Criptografia
    - Exercicios Cryptopals

Dimensao de Identidade - semana 4 
- OAuth 2.0 + OIDC + Keycloack
    - Aulas 8-10
    - Assistir apresentacao
    - Fazer exercicios
- OpenID Federation
    - Assistir treinamento

Dimensao IA - semana 5
- Agentes de IA
    - Assistir video
    - Ler materiais (1,2,3)
- LM Studio
    - Assistir video
    - Pratica

Dimensao Academica - semana 6
- Como ler artugos academicos
    - Ler artigos
    - Exercicio: esolher um artigo, responder perguntas.
